...
type
  ...
  public
    ...
    class function Execute(AConn: TFDCustomConnection; const ACaption: String; const APrompt: String;
      const ACustomFrm: TfrmFDGUIxFormsConnEdit = nil): Boolean; overload;
    class function Execute(var AConnStr: String; const ACaption: String; const APrompt: String;
      const ACustomFrm: TfrmFDGUIxFormsConnEdit = nil): Boolean; overload;
    class function Execute(AConnDef: IFDStanConnectionDef; const ACaption: String; const APrompt: String;
      const ACustomFrm: TfrmFDGUIxFormsConnEdit = nil): Boolean; overload;
...
uses
  ...
  FireDAC.Stan.Consts, FireDAC.Stan.ResStrs, gnugettext;
...
begin
  ...
  if (FConnection <> nil) and FConnection.Temporary then
    Result.ResultConnectionDef.Pooled := False;
end;
...
begin
  try
    sDesc := FDManager.GetBaseDriverDesc(FDriverID);
  except
    ShowMessage(Format(S_FD_ConnEditNoDrv, [FDriverID]));
    Exit;
  end;
  sDesc := StringReplace(sDesc, ' ', '_', [rfReplaceAll]);
  FDShell(C_Prefix + sDesc + C_Suff, S_FD_LComp_Design);
end;
...
class function TfrmFDGUIxFormsConnEdit.Execute(AConn: TFDCustomConnection;
  const ACaption: String; const APrompt: String; const ACustomFrm: TfrmFDGUIxFormsConnEdit = nil): Boolean;
var
...
begin
  ...
  try
    ...
    oFrm.lblPrompt.Caption := APrompt;
    oFrm.cbServerID.Enabled := False;
    ...
    oFrm.tsOptions.TabVisible := False;
    oFrm.tsSQL.TabVisible := False;
    oFrm.tsInfo.TabVisible := False;
    oFrm.lbCnxDef.Visible := False;
    oFrm.cbConnectionDefName.Visible := False;
    oFrm.btnWiki.Visible := False;
...
begin
  ...
  try
    ...
    Result := Execute(oConn, ACaption, APrompt, ACustomFrm);
    if Result then
...
begin
  ...
  try
    oConn.Temporary := True;
    oConn.ResultConnectionDef.Params.Assign(AConnDef.Params);
    AConnDef.ReadOptions(oConn.FormatOptions, oConn.UpdateOptions,
      oConn.FetchOptions, oConn.ResourceOptions);
    sName := AConnDef.Name;
    Result := Execute(oConn, ACaption, APrompt, ACustomFrm);
    if Result then begin
      AConnDef.Params.Assign(oConn.ResultConnectionDef.Params);
